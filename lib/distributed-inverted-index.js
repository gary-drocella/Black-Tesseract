...
module.exports = class DistributedRAMBasedInvertedIdex {
    ...
    bulkInsert(index, documents, cb) {
        ...
        Promise.all(promises).then((values) => {
            ...
            res[ResultDocumentConstants.SUCCESS] = failedCount == 0;
    ...
    bulkDelete(index, documents, cb) {
        ...
        for(let i = 0; i < documents[QueryDocumentConstants.BULK_DELETE_DOCS].length; i++) {
            let currDoc = documents[QueryDocumentConstants.BULK_DELETE_DOCS][i];
            let deleteDoc = currDoc[QueryDocumentConstants.DELETE];
            let id = deleteDoc[QueryDocumentConstants.ID];
